...
var countMaxOrSubsets = function (nums) {
    ...
    return backStack(nums, maxOr, 0, 0)
}

function backStack(nums, maxOr, index, currentOr) {
    ...
    let include = backStack(nums, maxOr, index + 1, currentOr | nums[index])
    let exclude = backStack(nums, maxOr, index + 1, currentOr)
    return include + exclude
}

const nums = [2, 2, 2]
console.log(countMaxOrSubsets(nums)) // 6
